...
namespace RedContritio
{
	namespace Experiment
	{
		...
		void GaussianBlur(cv::Mat& src, cv::Mat& dst, cv::Size ksize, double sigmaX)
		{
			...
			cv::Mat _dst;
			cv::copyMakeBorder(src, _dst, ksize.height/2, ksize.height/2, ksize.width/2, ksize.width/2, cv::BORDER_REFLECT_101);

			cv::Mat kernel_x = cv::getGaussianKernel(ksize.width, 0);
			cv::Mat kernel_y = cv::getGaussianKernel(ksize.height, 0);
				...
				for(int j=0; j<dst.cols; ++j)
				{
					...
					for(int m=0; m<ksize.height; ++m)
					{
						for(int n=0; n<ksize.width; ++n)
						{
							for(int k=0; k<3; ++k)
							{
								v[k] += _dst.at<cv::Vec3b>(i+m, j+n)[k] * kernel.at<double>(m, n);
							}
						}
					}

					for(int k=0; k<3; ++k) dst.at<cv::Vec3b>(i, j)[k] = cv::saturate_cast<uchar>(v[k]);
				}
			}
		}
